...
import { 
  ...
  logProctoringEvent,
  logCommonProctoringEvent
} from "../../services/examService";
import ProctoringMonitor from "../../components/ProctoringMonitor/ProctoringMonitor";
import "./ExamPage.css";
...
const ExamPage = () => {
  ...
  useEffect(() => {
    const fetchExam = async () => {
      try {
        ...
        // Log exam start using new API
        const candidateId = data.candidateId?._id || data.candidate?._id;
        if (candidateId) {
  ...
  const handleAnswerChange = async (questionIndex, value, type) => {
    ...
    // Log answer activity using new API
    if (exam) {
  ...
  const handleDisqualify = async (totalViolations, totalWarnings) => {
    ...
    // Log disqualification using new API
    if (exam) {
      const candidateId = exam.candidateId?._id || exam.candidate?._id;
      if (candidateId) {
        try {
          await logProctoringEvent(
            ...
            candidateId,
            'CHEATING_SUSPECTED',
            'Candidate disqualified due to excessive proctoring violations',
            'HIGH',
            { 
              totalViolations,
              totalWarnings,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Log submission attempt using new API
      if (exam) {
        const candidateId = exam.candidateId?._id || exam.candidate?._id;
        if (candidateId) {
          await logCommonProctoringEvent(
      ...
      // Log successful submission using new API
      if (exam) {
        const candidateId = exam.candidateId?._id || exam.candidate?._id;
        if (candidateId) {
          await logCommonProctoringEvent(
      ...
      // Log submission failure using new API
      if (exam) {
        const candidateId = exam.candidateId?._id || exam.candidate?._id;
        if (candidateId) {
          await logProctoringEvent(
